...
//streamName => event sourcing’in uygulandığı aggregate’e göre isimlendirilebilir.Burada örnek olarak bir sipariş sistemi için ‘order - stream’ adı verilmiştir.Eğer ki, ürün bazlı event sourcing yapılıyor olsaydı o takdirde ‘product - 123 - stream’, ‘product - 322 - stream’ şeklinde hususi isimlendirmeler de yapılark operasyonlar yürütülebilir.
//expectedState => bir stream’e event eklenirken bu akışın önceden oluşturulup oluşturulmadığını kontrol etmemizi sağlayan özelliktir.
//StreamState.NoStream : Belirtilen akışın daha önce oluşturulmadığını kontrol eder.
//StreamState.Any : Kontrola bulunmaz.
//StreamState.StreamExists : Belirtilen akışın daha önce oluşturulduğunu kontrol eder.
//
...
////    streamName: "order-stream", 
////    direction: Direction.Forwards, => Okuma yöntemini belirler.Direction.Forwads, olayları baştan sona doğru okur.Direction.Backwards ise ters yönde ...
////    revision: StreamPosition.Start => Stream'de ki olayları nereden yani kaçıncı event'ten başlayarak okuyacağını ifade eder Sayısal değer de verilebilir. 
////    );
